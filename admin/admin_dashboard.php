<?php
    ...
    $best_seller_query = mysqli_query($conn, "SELECT i.id AS item_id, i.title AS item_title, i.brand AS item_brand, i.photo AS item_photo, c.name AS category_name, SUM(i.price * oi.qty) AS total_price FROM tbl_item i JOIN tbl_order_items oi ON i.id = oi.item_id JOIN tbl_category c ON i.category_id = c.id GROUP BY i.id LIMIT 1;
");
